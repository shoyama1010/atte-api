            ...
            <tr>
                ...
                <td>
                    <input type="time" name="rests[0][break_start]"
                        value="{{ optional($attendance->rests->first())->break_start ? \Carbon\Carbon::parse($attendance->rests->first()->break_start)->format('H:i') : '' }}">
                    〜
                    <input type="time" name="rests[0][break_end]"
                        value="{{ optional($attendance->rests->first())->break_end ? \Carbon\Carbon::parse($attendance->rests->first()->break_end)->format('H:i') : '' }}">

                    @error('rests.0.break_start') <p class="error">{{ $message }}</p> @enderror
                    @error('rests.0.break_end') <p class="error">{{ $message }}</p> @enderror
                </td>
            </tr>
