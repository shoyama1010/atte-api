...
- 勤怠詳細・修正画面では複数の休憩時間を表示・入力できるようにし、出退勤時間だけでなく、複数の休憩時間についても修正申請できる構成としています。
- 修正申請後は「承認待ち」として管理し、管理者が申請内容を確認して承認することで、修正内容を勤怠データへ反映する承認フローを実装しました。
...
# 今後の改善
- 複数休憩の修正申請・承認は実装済みですが、申請データの保持方法をより一貫させるため、休憩修正内容を申請単位で1対多管理する設計への改善を検討しています。また、承認処理全体をDBトランザクションで囲み、勤怠・休憩・申請状態の更新をより安全に行えるよう改善予定です。
- 承認処理全体をDBトランザクションで囲み、勤怠・休憩・申請状態の更新をより安全に行えるよう改善する予定です。
- 修正申請処理について、複数休憩の追加・変更・削除を含むデータ更新処理をさらに整理し、保守性を高めることを検討しています。
